...
function renderAssociationCard(assoc) {
  ...
  const requiredMembers = assoc.maxMembers;
  const membersCount = assoc.currentMembers || 0;
    ...
    <div class="card">
      <div class="p-5 space-y-3 flex flex-col h-full">
        ...
        <div class="space-y-2 flex-grow">
          <p class="flex justify-between"><span class="text-gray-500">المبلغ الشهري:</span> <span>${assoc.monthlyAmount} ر.س</span></p>
          <p class="flex justify-between"><span class="text-gray-500">تاريخ البدء:</span> <span>${startDate}</span></p>
          <p class="flex justify-between"><span class="text-gray-500">نوع الجمعية:</span> <span>${assoc.type}</span></p>
          <p class="flex justify-between"><span class="text-gray-500">الحالة:</span> <span class="${assoc.status === 'active' ? 'text-green-600' : 'text-yellow-600'}">${assoc.status === 'active' ? 'نشطة' : 'قيد الانتظار'}</span></p>
          <p class="flex justify-between"><span class="text-gray-500">الأعضاء:</span> <span>${membersCount} / ${requiredMembers}</span></p>
        </div>
        <div class="mt-auto flex flex-wrap gap-2 justify-between">
          ...
          <button onclick="testAssociationCycle(${assoc.id}, ${requiredMembers})"
                  class="action-btn bg-yellow-100 text-yellow-700 hover:bg-yellow-200">اختبار الدورة</button>
        </div>
      </div>
    </div>
  `;
...
async function loadAssociations() {
  ...
  try {
    ...
    // Fetch members for each association to get current count
    const assocsWithMembers = await Promise.all(data.data.map(async assoc => {
      try {
        const membersRes = await axios.get(`https://api.technologytanda.com/api/associations/${assoc.id}/members`);
        assoc.currentMembers = membersRes.data?.data?.length || 0;
      } catch (e) {
        assoc.currentMembers = 0;
      }
      return assoc;
    }));
    
    container.innerHTML = assocsWithMembers.map(renderAssociationCard).join('');
  } catch (e) {
...
document.getElementById('createUserForm').onsubmit = async e => {
  ...
  try {
    ...
    alert("حدث خطأ أثناء إنشاء المستخدم: " + (error.response?.data?.message || error.message));
  }
};
...
document.getElementById('addMemberForm').onsubmit = async e => {
  ...
  try {
    ...
    closeAddMemberModal();
    loadAssociations(); // Refresh associations to update member count
  } catch (error) {
...
document.getElementById('notificationForm').onsubmit = async e => {
  ...
  const associationId = document.getElementById('notificationAssociationSelect').value;
...
// Test association cycle
async function testAssociationCycle(assocId, requiredMembers) {
  try {
    // Fetch current members
    const membersRes = await axios.get(`https://api.technologytanda.com/api/associations/${assocId}/members`);
    const currentMembers = membersRes.data?.data?.length || 0;
    
    // Validate member count
    if (currentMembers < requiredMembers) {
      alert(`يجب أن يكون عدد الأعضاء ${requiredMembers} لبدء دورة الاختبار. حالياً العدد هو ${currentMembers}.`);
      return;
    }
    
    if (!confirm("هل تريد حقاً تشغيل دورة اختبارية لهذه الجمعية؟")) return;
    
    const res = await axios.post(`https://api.technologytanda.com/api/associations/test-cycle`, {
      associationId: assocId
    });
    
    if (res.data.success) {
      alert(`تم تنفيذ الدورة بنجاح:\n${res.data.logs.join('\n')}`);
      loadAssociations(); // Refresh the associations to show updated status
    }
  } catch (error) {
