// Function to show custom modal
        ...
        if (!token) {
            window.location.href = "https://jmaia-app.github.io/Jmaia/login.html";
        }
        ...
        async function loadProfile() {
            try {
                ...
                if (data.profileImage) {
                    const imageUrl = `http://localhost:3000/uploads/${data.profileImage.replace('uploads/', '')}?t=${new Date().getTime()}`;
                    imgEl.src = imageUrl;
        ...
        document.getElementById('profile-image-input').addEventListener('change', async function() {
            ...
            try {
                const res = await fetch('http://localhost:3000/api/userData/user/update', {
                    method: 'PUT',
                    headers: {
                ...
                const newImgUrl = `http://localhost:3000/uploads/${data.user.profileImage.replace('uploads/', '')}?t=${Date.now()}`;
